	...
	<div class="row-fluid">
		...
		<div class="tweets hero-unit span9">
			...
			%for tweet in timeline:
				<p><img src="/static/avatar.png" /> <strong><a href="/{{tweet.user.username}}">{{tweet.user.username}}</a></strong> {{tweet.content}}<span><a href="/{{tweet.user.username}}/statuses/{{tweet.id}}">permalink</a>

				%if tweet.user.username == username:
					<a href="/home/delete/{{tweet.id}}">delete</a>
				%end
				</span></p>
			%end
		</div>
